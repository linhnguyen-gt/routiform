/**
 * Routiform Electron Desktop App - Main Process
 *
 ...
 * #14 Removed dead routiform:// protocol (no handler existed)
...
function resolveDataDir(overridePath, env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return path.join(appData, "routiform");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return path.join(path.resolve(xdg), "routiform");

  return path.join(require("os").homedir(), ".routiform");
}
...
function setupContentSecurityPolicy() {
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    const csp = [
      "default-src 'self'",
      `connect-src 'self' http://localhost:* ws://localhost:* https://*.routiform.online https://*.routiform.dev`,
...
function startNextServer() {
  ...
  if (changed) {
    serverEnv.ROUTIFORM_BOOTSTRAPPED = "true";
    try {
      fs.mkdirSync(dataDir, { recursive: true });
      const lines = [
        "# Auto-generated by Routiform bootstrap",
        "",
