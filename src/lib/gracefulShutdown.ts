...
declare global {
  var __routiformShutdown:
    | { init: boolean; shuttingDown: boolean; activeRequests: number }
    | undefined;
}

function getShutdownState() {
  if (!globalThis.__routiformShutdown) {
    globalThis.__routiformShutdown = { init: false, shuttingDown: false, activeRequests: 0 };
  }
  return globalThis.__routiformShutdown;
}
