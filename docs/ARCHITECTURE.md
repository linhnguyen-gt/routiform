...
| `CodexExecutor`       | OpenAI Codex                                                                                                                                                                                  | Uses shared token-refresh mutex path, converts native passthrough `input` role `system` → `developer`, injects fallback instructions when missing, strips unsupported token-cap aliases |
