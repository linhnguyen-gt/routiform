# Routiform Fly.io 部署指南 (Português (Portugal))
...
如果当前仓库是 fork，并且你要同步上游 `https://github.com/linhnguyen-gt/Routiform` 的更新，推荐按下面流程执行。
...
```powershell
git remote add upstream https://github.com/linhnguyen-gt/Routiform.git
```
